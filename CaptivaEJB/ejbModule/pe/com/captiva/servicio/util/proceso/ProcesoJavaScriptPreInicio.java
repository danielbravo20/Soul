...
public class ProcesoJavaScriptPreInicio extends MultipleBaseConstructor{
	...
	public List<Componente> crear(ProyectoBean proyectoBean) throws Exception{
		...
		for (int x = 0; x < procesos.size(); x++) {
			...
			if(procesoBean!=null){
				String directorio = proyectoBean.getEquipoBean().getDirectorioWorkspace() + "\\" + proyectoBean.getProyecto() + ProyectoBean.SUFIJO_PROYECTO_WEB + "\\WebContent\\portal\\procesos\\" + procesoBean.getAleas() + "\\proceso\\preControlador\\" ;
