...
public class TareaClaseServicio extends MultipleBaseConstructor{
	...
	public List<Componente> crear(ProyectoBean proyectoBean) throws Exception{
		...
		for (int x = 0; x < procesos.size(); x++) {
			...
			if(procesoBean!=null){
				String directorio = proyectoBean.getEquipoBean().getDirectorioWorkspace() + "\\" + proyectoBean.getProyecto() + ProyectoBean.SUFIJO_PROYECTO_SERVICE + "\\" + "ejbModule" + "\\" + (proyectoBean.getPaquete().replace('.', File.separatorChar) + "\\" + procesoBean.getClase().toLowerCase() + "\\tarea\\servicio\\");
				...
				for (int y = 0; y < tareas.size(); y++) {
					...
					componente.setArchivo(tareaBean.getClase()+"Servicio.java");
	...
	private StringBuffer contenido(ProyectoBean proyectoBean, ProcesoBean procesoBean, TareaBean tareaBean){
		...
		buffer.append("public class "+tareaBean.getClase()+"Servicio extends Pre"+tareaBean.getClase()+"Servicio implements "+tareaBean.getClase()+"ServicioLocal {\r\n\r\n");
