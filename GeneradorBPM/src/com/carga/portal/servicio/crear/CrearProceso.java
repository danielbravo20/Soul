...
public class CrearProceso {
	...
	public void crear(Proyecto proyecto, Configuracion configuracion, Connection connection) throws Exception{
		...
		//Archivo de configuraci�n de enlace ejb
	...
	private String contenidoArchivoClase(Proyecto proyecto, Proceso proceso, Connection connection) throws Exception{
		...
		buffer.append("\t\t"+clasePadreDetalle.toLowerCase()+".setHistoricoObservaciones("+nombreEJBDaoCliente+".consultarObservaciones("+clasePadreDetalle.toLowerCase()+".getPiid()));\r\n");
		//buffer.append("\t\t"+clasePadreDetalle.toLowerCase()+".setExcepciones("+nombreEJBDaoCliente+".consultarExcepcionxObservacion("+clasePadreDetalle.toLowerCase()+".getHistoricoObservaciones()));\r\n");
	...
	private String contenidoArchivoClaseDao(Proceso proceso, Connection conn) throws Exception{
		...
		int i = 0;
		List<ClaseProceso> clasesProceso = Validaciones.filtrarClases(proceso.getAtributosEntrada());
		for (int x = 0; x < clasesProceso.size(); x++) {
			Clase clase = clasesProceso.get(x);
			String paquete = clase.getJavaPaquete();
			String nombreClase = clase.getJavaClase();
			if(map.containsKey(paquete+nombreClase)==false){
				map.put(paquete+nombreClase, proceso.getAtributosEntrada().get(x));
				buffer.append("import "+paquete+"."+nombreClase+";\r\n");
				if(i!=0){
					parametros.append(", ");
				}
				parametros.append(nombreClase+" "+nombreClase.toLowerCase());
				i++;
			}
		}
	...
	private String contenidoArchivoClaseDaoImpl(Proyecto proyecto, Proceso proceso, Connection conn) throws Exception{
		...
		buffer.append(contenidoDaoImplObservarComunFuncion(proyecto));
	...
	private String contenidoDaoImplObservarComunFuncion(Proyecto proyecto){
