...
public class CrearSQL {
	...
	public void crear(Proyecto proyecto, Configuracion configuracion, Connection connection) throws Exception{
		...
		//Nexo - se comenta xq no esta actualizado
	...
	public static String spProcesosRollback(Proyecto proyecto, Tabla tabla, Connection conn) throws Exception{
		...
		if (procesos != null){
			for(int x=0;x<procesos.size(); x++){
				...
				sb.append(spProcesosContenidoSecuencialDocumentoDrop(proyecto, proceso));
	...
	private static String spProcesosContenidoSecuencialDocumentoDrop(Proyecto proyecto, Proceso proceso){
		Tabla tabla = proyecto.getTablas().get(0);
		StringBuffer buffer = new StringBuffer();
		buffer.append("DROP PROCEDURE "+tabla.getEsquema()+".SP_PORTAL_PRO_"+proceso.getClase().toUpperCase()+"_SECUENCIA_DOCUMENTO@\r\n\r\n");
		return buffer.toString();
	}
	...
	private static String spTareasContenidoArchivoDLLCancelar(Tarea tarea) throws Exception{
		...
		if(atributosPKs.size()==0){
			throw new Exception("No se puede armar el Store Procedure de cancelaci�n, por que en el excel de configuraci�n de la tarea "+tarea.getCodigo()+" no se incluyo un PK, por favor ingrese un PK y vuelva a intentarlo...");
		}
	...
	public static void crearInsert(Proyecto proyecto, Configuracion configuracion, File directorioScriptCreate, File directorioScriptDrop, Connection connection) throws Exception{
		...
		File archivo = new File(directorioScriptCreate.getAbsolutePath()+"\\Nexo\\","3_INSERT_NEXO.sql");
		if(archivo.exists()){
			archivo.delete();
		}
		archivo.createNewFile();
		...
		*/
		...
		File archivoDrop = new File(directorioScriptDrop.getAbsolutePath()+"\\Nexo\\","1_DELETE_NEXO.sql");
		if(archivoDrop.exists()){
			archivoDrop.delete();
		}
	...
	public void integrarArchivos(Configuracion configuracion) throws Exception{
		
		//Script del proceso
		File directorioScriptInicio = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\create\\Inicio");
		StringBuffer contenidoCreate = unirArchivos(directorioScriptInicio);
		
		//Script del proceso
		File directorioScriptCreate = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\create");
		contenidoCreate.append(unirArchivos(directorioScriptCreate));
		
		//Script del personalizado
		File directorioScriptPersonalizado = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\create\\Personalizado");
		contenidoCreate.append(unirArchivos(directorioScriptPersonalizado));
		
		//Script del mantenimiento
		File directorioScriptMantenimiento = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\create\\Mantenimiento");
		contenidoCreate.append(unirArchivos(directorioScriptMantenimiento));
		
		//Script del mantenimiento
		File directorioScriptReportes = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\create\\Reportes");
		contenidoCreate.append(unirArchivos(directorioScriptReportes));
		
		crearArchivo(directorioScriptCreate.getAbsolutePath()+"\\", "BFP_BE_CREATE.SQL", contenidoCreate);
		
		//Script del proceso
		File directorioScriptRollback = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\rollback");
		StringBuffer contenidoRollback = unirArchivos(directorioScriptRollback);
		
		//Script del mantenimiento
		File directorioScriptRollbackPersonalizado = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\rollback\\Personalizado");
		contenidoRollback.append(unirArchivos(directorioScriptRollbackPersonalizado));

		//Script del mantenimiento
		File directorioScriptRollbackMantenimiento = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\rollback\\Mantenimiento");
		contenidoRollback.append(unirArchivos(directorioScriptRollbackMantenimiento));
		
		//Script del reportes
		File directorioScriptRollbackReportes = new File(configuracion.getDirectorioSQL().getAbsoluteFile()+"\\script\\rollback\\Reportes");
		contenidoRollback.append(unirArchivos(directorioScriptRollbackReportes));
		
		crearArchivo(directorioScriptRollback.getAbsolutePath()+"\\", "BFP_BE_ROLLBACK.SQL", contenidoRollback);
		
	}
	
	public StringBuffer unirArchivos(File directorio) throws Exception{
			...
			if(files!=null && files.length >0){
				for (int i = 0; i < files.length; i++) {
					if(files[i].isDirectory()==false){
						BufferedReader bf = new BufferedReader(new InputStreamReader(new FileInputStream(files[i]), "ISO-8859-1"));
						String linea = null;
				...
				throw new Exception("El directorio no cuenta con archivos o directorios...");
			}
		}else{
			throw new Exception("La ruta indicada no existe o no es un directorio...");
		}
		
		return contenido;
	}
	
	public void crearArchivo(String ruta, String nombre, StringBuffer contenido) throws Exception{
