...
public class CrearTarea {
	...
	private String contenidoArchivoClase(Proyecto proyecto, Tarea tarea, Connection connection) throws Exception{
		...
		*/
		...
		buffer.append("\tpublic final static String CODIGO_REQUEST_TIEMPO_ROJO = \"tiempoRojo\";\r\n");
		buffer.append("\tpublic final static String CODIGO_REQUEST_TIEMPO_AMARRILLO = \"tiempoAmarrillo\";\r\n\r\n");
		buffer.append("\t@EJB\r\n");
		buffer.append("\t"+tarea.getClase()+"Dao "+tarea.getClase().toLowerCase()+"Dao;\r\n");
...
/*
	...
	private String contenidoMetodoGuardarMensajeEntrada(Tarea tarea, Connection conn){
		...
		for (int i = 0; i < tarea.getObjetosBPMTrabajar().size(); i++) {
			...
			if(objetosBPM.getAtributosTarea()!=null && objetosBPM.getAtributosTarea().size()>0){
				...
				if(objetosBPM.getClase()!=null && tarea.getClasePadre().getJavaClase().equalsIgnoreCase(objetosBPM.getClase().getJavaClase())==false){
	...
	private String contenidoMetodoHistorial(Tarea tarea) throws Exception {
		...
		buffer.append("\tpublic Object registrarHistorialTarea(HttpServletRequest request, Task task) throws Exception{\r\n");
		...
		//TODO dbravo revisar logica para completar una tarea
		buffer.append("\t\tString cadenaTiempoAmarillo = request.getParameter(CODIGO_REQUEST_TIEMPO_AMARRILLO);\r\n");
		buffer.append("\t\tString cadenaTiempoRojo = request.getParameter(CODIGO_REQUEST_TIEMPO_ROJO);\r\n");
		buffer.append("\t\tif(cadenaTiempoAmarillo!=null && cadenaTiempoRojo!=null){\r\n");
		buffer.append("\t\t\tlong tiempoAmarillo = Long.parseLong(cadenaTiempoAmarillo);\r\n");
		buffer.append("\t\t\tlong tiempoRojo = Long.parseLong(cadenaTiempoRojo);\r\n");
		buffer.append("\t\t\tlong tiempo = fechaCreacionTermino.getTimeInMillis() - fechaCreacionInicio.getTimeInMillis();\r\n");
		buffer.append("\t\t\tif(tiempo<tiempoAmarillo){\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setCodigoResultado(\"1\");\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setDescripcionResultado(\"VERDE\");\r\n");
		buffer.append("\t\t\t}else if(tiempo>tiempoRojo){\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setCodigoResultado(\"3\");\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setDescripcionResultado(\"ROJO\");\r\n");
		buffer.append("\t\t\t}else{\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setCodigoResultado(\"2\");\r\n");
		buffer.append("\t\t\t\ttareaHistorial.setDescripcionResultado(\"AMBAR\");\r\n");
		buffer.append("\t\t\t}\r\n");
		buffer.append("\t\t}\r\n");
		
		buffer.append("\t\t"+tarea.getClase().toLowerCase()+"Dao.registraHistorial(tareaHistorial);\r\n");
		buffer.append("\treturn tareaHistorial;\r\n");
		buffer.append("\t}\r\n");
		return buffer.toString();
	}
	...
	private String contenidoArchivoClaseUtil(Proyecto proyecto, Tarea tarea, Connection connection) throws Exception{
		...
		List<Clase> clases = Validaciones.filtrarClasesTarea(tarea.getAtributosCompletarSinPK());
		...
		for (int x = 0; x < tarea.getAtributosCompletar().size(); x++) {
			...
			if(atributo.isRequiereValidacion() && (atributo.getWebValorOmision()==null || atributo.getWebValorOmision().trim().length()==0)){
				...
				if(atributo.getWebNombre()==null || atributo.getWebNombre().trim().equals("")){
					throw new Exception("El atributo "+atributo.getCodigo()+" "+atributo.getNombre()+", no tiene un nombre web...");
				}
